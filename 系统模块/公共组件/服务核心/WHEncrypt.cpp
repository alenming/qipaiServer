...
//des加密
bool CWHEncrypt::DesEncrypt(LPCTSTR pszSourceData,LPTSTR pszEncrypData)
{
	return true;
}

//des解密
bool CWHEncrypt::DesCrevasse(LPCTSTR pszEncrypData,LPTSTR pszSourceData)
{
	ASSERT((pszEncrypData!=NULL)&&(pszSourceData!=NULL));

	yxyDES des;
	des.InitializeKey("22waWA!@");

	CT2CA strEncrypData(pszEncrypData);
	des.DecryptAnyLength((char*)strEncrypData);

	char * szR = des.GetPlaintextAnyLength();
	USES_CONVERSION;
	lstrcpyn(pszSourceData,A2T(szR),MAX_PATH);
