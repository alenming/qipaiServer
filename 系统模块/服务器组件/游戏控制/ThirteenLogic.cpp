...
//扑克数据
const BYTE	CThirteenLogic::m_bCardListData[52] =
{
...
//获取类型
BYTE CThirteenLogic::GetCardType(BYTE bCardData[], BYTE bCardCount)
{
	//数据校验
	ASSERT(bCardCount==3 || bCardCount==5 );
	if (bCardCount != 3 && bCardCount != 5) return TH_INVALID;
	...
	//分析扑克
	tagTHAnalyseResult AnalyseResult;
	...
	case 5:	//五张牌型
		{
			...
			if (!bLine)
			{
				if (GetCardLogicValue(bCardData[0]) == 14)
				{
					bLine = true;
					for (BYTE i = 1; i<4; ++i)
					{
			...
			if (false == bLine)
			{
				...
				{
					return TH_FIVE_FLUSH;
				}
			}
			else if (true == bLine)
			{
				//杂顺类型
				if (false == AnalyseData.bSameColor) return TH_FIVE_SINGLE_LINE;
				//同花顺牌
				else							  return TH_FIVE_TONGHUASHUN;
			}

			//四带单张
			if(1==AnalyseData.bFourCount && 1==AnalyseData.bOneCount)
			{
				return	 TH_FIVE_FOUR_ONE;
			}

			//三条一对
			if(1==AnalyseData.bThreeCount && 1==AnalyseData.bTwoCount)
			{
				return	 TH_FIVE_THREE_DEOUBLE;
			}
...
//判断特殊牌型
//必须是13张
BYTE CThirteenLogic::GetSpecialType(BYTE bHandCardData[], BYTE bCardCount)
{
	ASSERT(bCardCount == HAND_CARD_COUNT);
	if (bCardCount != HAND_CARD_COUNT) return TH_INVALID;
	...
	if (IsStraightDragon(bCardData, bCardCount)==true)
		return TH_EX_ZHIZUNQINGLONG;

	//一条龙
	if ((IsLinkCard(bCardData, bCardCount)==true))
		return TH_EX_YITIAOLONG;

	//十二皇族
	if (bShiErHuang)
		return TH_EX_SHIERHUANGZU;
...
//bSpecCardData[][]
//顺序为前中后
BYTE CThirteenLogic::GetSpecialCardData(BYTE bHandCardData[HAND_CARD_COUNT], BYTE bHandCardCount, BYTE bSpecCardData[HAND_CARD_COUNT])
{
...
//排列扑克
void CThirteenLogic::SortCardList(BYTE bCardData[], BYTE bCardCount, enSortCardType SortCardType)
{
...
//混乱扑克
void CThirteenLogic::RandCardList(BYTE bCardBuffer[], BYTE bBufferCount)
{
	//混乱准备
...
//删除扑克
bool CThirteenLogic::RemoveCard(const BYTE bRemoveCard[], BYTE bRemoveCount, BYTE bCardData[], BYTE cbTemp[], BYTE bCardCount)
{
	//检验数据
...
void CThirteenLogic::AutoPutCard(BYTE bCardData[], BYTE bPutCardData[])
{
...
int CThirteenLogic::AutoSixPutCard(BYTE bCardData[], BYTE bPutCardData[][13])
{
	BYTE bTempCard[6][3][5]; //保存权值最大的六副牌型
	int  iTempWeights[6][3]; //保存权值最大的六副牌型的权值
	BYTE bCardList[13];      //保存手牌
...
//获取特殊牌型倍数
int CThirteenLogic::nGetSpecialTimes(BYTE cbFirstType, BYTE cbNextType)
{
	int nMulOnSD = 0;
	//至尊清龙
	if ((cbFirstType == TH_EX_ZHIZUNQINGLONG || cbNextType == TH_EX_ZHIZUNQINGLONG))
	{
		if (cbFirstType == TH_EX_ZHIZUNQINGLONG && cbNextType < TH_EX_ZHIZUNQINGLONG)
			nMulOnSD = 108;
		else if (cbFirstType < TH_EX_ZHIZUNQINGLONG && cbNextType == TH_EX_ZHIZUNQINGLONG)
			nMulOnSD = -108;
	}
	//一条龙
	else if ((cbFirstType == TH_EX_YITIAOLONG || cbNextType == TH_EX_YITIAOLONG))
	{
		int nMulOnSD = 0;
		if (cbFirstType == TH_EX_YITIAOLONG && cbNextType < TH_EX_YITIAOLONG)
			nMulOnSD = 36;
		else if (cbFirstType < TH_EX_YITIAOLONG && cbNextType == TH_EX_YITIAOLONG)
			nMulOnSD = -36;
	}
	//十二皇族
	else if ((cbFirstType == TH_EX_SHIERHUANGZU || cbNextType == TH_EX_SHIERHUANGZU))
	{
		int nMulOnSD = 0;
		if (cbFirstType == TH_EX_SHIERHUANGZU && cbNextType < TH_EX_SHIERHUANGZU)
			nMulOnSD = 24;
		else if (cbFirstType < TH_EX_SHIERHUANGZU && cbNextType == TH_EX_SHIERHUANGZU)
			nMulOnSD = -24;
	}
	//三同花顺
	else if ((cbFirstType == TH_EX_SANTONGHUASHUN || cbNextType == TH_EX_SANTONGHUASHUN))
	{
		int nMulOnSD = 0;
		if (cbFirstType == TH_EX_SANTONGHUASHUN && cbNextType < TH_EX_SANTONGHUASHUN)
			nMulOnSD = 20;
		else if (cbFirstType < TH_EX_SANTONGHUASHUN && cbNextType == TH_EX_SANTONGHUASHUN)
			nMulOnSD = -20;
	}
	//三分天下
	else if ((cbFirstType == TH_EX_SANFENGTIANXIA || cbNextType == TH_EX_SANFENGTIANXIA))
	{
		int nMulOnSD = 0;
		if (cbFirstType == TH_EX_SANFENGTIANXIA && cbNextType < TH_EX_SANFENGTIANXIA)
			nMulOnSD = 20;
		else if (cbFirstType < TH_EX_SANFENGTIANXIA && cbNextType == TH_EX_SANFENGTIANXIA)
			nMulOnSD = -20;
	}
	//全大
	else if ((cbFirstType == TH_EX_QUANDA || cbNextType == TH_EX_QUANDA))
	{
		int nMulOnSD = 0;
		if (cbFirstType == TH_EX_QUANDA && cbNextType < TH_EX_QUANDA)
			nMulOnSD = 10;
		else if (cbFirstType < TH_EX_QUANDA && cbNextType == TH_EX_QUANDA)
			nMulOnSD = -10;
	}
	//全小
	else if ((cbFirstType == TH_EX_QUANXIAO || cbNextType == TH_EX_QUANXIAO))
	{
		int nMulOnSD = 0;
		if (cbFirstType == TH_EX_QUANXIAO && cbNextType < TH_EX_QUANXIAO)
			nMulOnSD = 10;
		else if (cbFirstType < TH_EX_QUANXIAO && cbNextType == TH_EX_QUANXIAO)
			nMulOnSD = -10;
	}
	//凑一色
	else if ((cbFirstType == TH_EX_COUYISE || cbNextType == TH_EX_COUYISE))
	{
		int nMulOnSD = 0;
		if (cbFirstType == TH_EX_COUYISE && cbNextType < TH_EX_COUYISE)
			nMulOnSD = 10;
		else if (cbFirstType < TH_EX_COUYISE && cbNextType == TH_EX_COUYISE)
			nMulOnSD = -10;
	}
	//四套三条
	else if ((cbFirstType == TH_EX_SITAOSANTIAO || cbNextType == TH_EX_SITAOSANTIAO))
	{
		int nMulOnSD = 0;
		if (cbFirstType == TH_EX_SITAOSANTIAO && cbNextType < TH_EX_SITAOSANTIAO)
			nMulOnSD = 6;
		else if (cbFirstType < TH_EX_SITAOSANTIAO && cbNextType == TH_EX_SITAOSANTIAO)
			nMulOnSD = -6;
	}
	//五对三条
	else if ((cbFirstType == TH_EX_WUDUISANTIAO || cbNextType == TH_EX_WUDUISANTIAO))
	{
		int nMulOnSD = 0;
		if (cbFirstType == TH_EX_WUDUISANTIAO && cbNextType < TH_EX_WUDUISANTIAO)
			nMulOnSD = 5;
		else if (cbFirstType < TH_EX_WUDUISANTIAO && cbNextType == TH_EX_WUDUISANTIAO)
			nMulOnSD = -5;
	}
	//六对半
	else if ((cbFirstType == TH_EX_LIUDUIBAN || cbNextType == TH_EX_LIUDUIBAN))
	{
		int nMulOnSD = 0;
		if (cbFirstType == TH_EX_LIUDUIBAN && cbNextType < TH_EX_LIUDUIBAN)
			nMulOnSD = 4;
		else if (cbFirstType < TH_EX_LIUDUIBAN && cbNextType == TH_EX_LIUDUIBAN)
			nMulOnSD = -4;
	}
	//三顺子
	else if ((cbFirstType == TH_EX_SANSHUNZI || cbNextType == TH_EX_SANSHUNZI))
	{
		int nMulOnSD = 0;
		if (cbFirstType == TH_EX_SANSHUNZI && cbNextType < TH_EX_SANSHUNZI)
			nMulOnSD = 4;
		else if (cbFirstType < TH_EX_SANSHUNZI && cbNextType == TH_EX_SANSHUNZI)
			nMulOnSD = -4;
	}
	//三同花
	else if ((cbFirstType == TH_EX_SANTONGHUA || cbNextType == TH_EX_SANTONGHUA))
	{
		int nMulOnSD = 0;
		if (cbFirstType == TH_EX_SANTONGHUA && cbNextType < TH_EX_SANTONGHUA)
			nMulOnSD = 3;
		else if (cbFirstType < TH_EX_SANTONGHUA && cbNextType == TH_EX_SANTONGHUA)
			nMulOnSD = -3;
	}
...
//逻辑数值
BYTE CThirteenLogic::GetCardLogicValue(BYTE bCardData)
{
	//扑克属性
...
//比较扑克
int CThirteenLogic::CompareCard(BYTE bInFirstList[], BYTE bInNextList[], BYTE bFirstCount, BYTE bNextCount, bool bComperWithOther)
{
	//定义变量
	BYTE bFirstList[13], bNextList[13];
	tagTHAnalyseData FirstAnalyseData, NextAnalyseData;
	...
	//检查转化
	tagTHTransData ttdFst;
	tagTHTransData ttdNxt;
	...
	ASSERT(TH_INVALID != bNextType && TH_INVALID != bFirstType);
	if (TH_INVALID == bFirstType || TH_INVALID == bNextType) return enCRError;
	...
	if(true==bComperWithOther)
	{
		...
		if(bNextType==bFirstType)
		{
			...
			case TH_FIVE_THREE_DEOUBLE:			//三条一对
				{
					if (GetCardLogicValue(bNextList[NextAnalyseData.bThreeFirst[0]]) == GetCardLogicValue(bFirstList[FirstAnalyseData.bThreeFirst[0]]))
					{
						if (GetCardLogicValue(bNextList[NextAnalyseData.bTwoFirst[0]]) > GetCardLogicValue(bFirstList[FirstAnalyseData.bTwoFirst[0]]))
			...
			case TH_FIVE_SINGLE_LINE:		//顺子
			case TH_FIVE_FLUSH:					//同花五牌
				{
					//A5432特殊顺子比较
					if (bFirstType == TH_FIVE_SINGLE_LINE)
					{
	...
	{
		ASSERT(bFirstType == TH_SINGLE || bFirstType == TH_ONE_DOUBLE || bFirstType == TH_THREE);
		if (bFirstType != TH_SINGLE && bFirstType != TH_ONE_DOUBLE && bFirstType != TH_THREE)
			return enCRError;
		//开始对比
		if(bNextType==bFirstType)
		{
...
//是否顺子
bool CThirteenLogic::IsLinkCard(const BYTE cbCardData[], BYTE cbCardCount)
{
...
//是否同花
bool CThirteenLogic::IsSameColorCard(const BYTE cbCardData[], BYTE cbCardCount)
{
	ASSERT(cbCardCount > 0);
	if (cbCardCount <= 0) return false;
...
//是否同花顺
bool CThirteenLogic::IsStraightDragon(const BYTE cbCardData[], BYTE bCardCount)
{
	//校验数据
...
//获取对数
BYTE CThirteenLogic::GetDoubleCount(BYTE cbFrontCard[], BYTE cbMidCard[], BYTE cbBackCard[])
{
	tagTHAnalyseData AanlyseFront, AnalyseMid, AnalyeBack;
...
//设置顺子
bool CThirteenLogic::SetLinkCard(BYTE cbCardData[], BYTE cbCardCount, BYTE cbLineCardData[3][5])
{
	if (cbCardCount % 5 != 0 && cbCardCount % 5 != 3)
		return false;

	tagTHSearchCardResult LineCardResult;
...
//分析扑克
void CThirteenLogic::AnalyseCard(const BYTE bCardDataList[], const BYTE bCardCount, tagTHAnalyseData& AnalyseData)
{
...
//分析扑克
void CThirteenLogic::AnalysebCardData(const BYTE cbCardData[], BYTE cbCardCount, tagTHAnalyseResult & AnalyseResult)
{
	//设置结果
	ZeroMemory(&AnalyseResult,sizeof(AnalyseResult));
...
//分析分布
VOID CThirteenLogic::AnalysebDistributing(const BYTE cbCardData[], BYTE cbCardCount, tagTHDistributing & Distributing)
{
	//设置变量
...
false				前大于后
****************************************************/
bool CThirteenLogic::CompareOneCardEx(BYTE bFirstCard, BYTE bNextCard, tagTHTransData & ttdFst, tagTHTransData & ttdNxt)
{
	//牌值比较
...
****************************************************/
BYTE CThirteenLogic::GetMaxCardData(const BYTE cbCardData[], const BYTE cbCardCount, BYTE bMaxCardData[], BYTE & bMaxCardCount, BYTE bNeedCCount)
{
	//校验数据
...
*返回值：0					不转化
		 other				转化后的牌型
****************************************************/
BYTE CThirteenLogic::AnalyseMaxTransform(const BYTE cbCardData[], const BYTE cbCardCount, BYTE bTransCardData[], tagTHTransData & TransData)
{
	...
	if ( 5 == cbCardCount && tad.bSameColor &&	0 == (tad.bTwoCount+tad.bThreeCount+tad.bFourCount) )
	{
		...
		{
			TransformCard(bTempCardData, cbCardCount, bKcount, TH_FIVE_TONGHUASHUN, tad, bTransCardData, TransData);
			return TH_FIVE_TONGHUASHUN;
		}
	...
	{
		TransformCard(bTempCardData, cbCardCount, bKcount, TH_FIVE_FOUR_ONE, tad, bTransCardData, TransData);
		return TH_FIVE_FOUR_ONE;
	}
	...
	{
		TransformCard(bTempCardData, cbCardCount, bKcount, TH_FIVE_THREE_DEOUBLE, tad, bTransCardData, TransData);
		return TH_FIVE_THREE_DEOUBLE;
	}
	...
	{
		TransformCard(bTempCardData, cbCardCount, bKcount, TH_FIVE_FLUSH, tad, bTransCardData, TransData);
		return TH_FIVE_FLUSH;
	}
	...
	if ( 5 == cbCardCount && !tad.bSameColor &&	0 == (tad.bTwoCount+tad.bThreeCount+tad.bFourCount) )
	{
		...
		if (14 == GetCardLogicValue(bTempCardData[0]) && 5 >= GetCardLogicValue(bTempCardData[1]))
		{
			TransformCard(bTempCardData, cbCardCount, bKcount, TH_FIVE_SINGLE_LINE, tad, bTransCardData, TransData);
			return TH_FIVE_SINGLE_LINE;
		}
	...
	{
		TransformCard(bTempCardData, cbCardCount, bKcount, TH_THREE, tad, bTransCardData, TransData);
		return TH_THREE;
	}
	...
	{
		TransformCard(bTempCardData, cbCardCount, bKcount, TH_ONE_DOUBLE, tad, bTransCardData, TransData);
		return TH_ONE_DOUBLE;
	}
...
****************************************************/
void CThirteenLogic::TransformCard(const BYTE cbNkCardData[], BYTE bCardCount, BYTE bKCount, BYTE bCardType, const tagTHAnalyseData & tad,
	BYTE bTransCardData[], tagTHTransData & TransData)
{
	...
	case TH_FIVE_TONGHUASHUN:
	case TH_FIVE_SINGLE_LINE:
		{
			//数据校验
			ASSERT(5==bCardCount);
			...
			for (int i = 1; i < 5; i++)
			{
				...
				{
					BYTE transCard = 0;
					if (bCardType == TH_FIVE_TONGHUASHUN)
...
//同牌搜索
BYTE CThirteenLogic::SearchSameCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, BYTE cbSameCardCount, tagTHSearchCardResult *pSearchCardResult)
{
	//设置结果
	if( pSearchCardResult )
		ZeroMemory(pSearchCardResult, sizeof(tagTHSearchCardResult));
...
BYTE CThirteenLogic::SearchTakeCardType(const BYTE cbHandCardData[], BYTE cbHandCardCount, BYTE cbSameCount, BYTE cbTakeCardCount, tagTHSearchCardResult *pSearchCardResult)
{
	//设置结果
	if( pSearchCardResult )
		ZeroMemory(pSearchCardResult, sizeof(tagTHSearchCardResult));
	...
	//搜索同牌
	tagTHSearchCardResult SameCardResult;
	...
	{
		//搜索带牌
		tagTHSearchCardResult TakeCardResult;
...
//搜索同花
BYTE CThirteenLogic::SearchSameColorType(const BYTE cbHandCardData[], BYTE cbHandCardCount, BYTE cbSameCount, tagTHSearchCardResult *pSearchCardResult)
{
	//设置结果
	if( pSearchCardResult )
		ZeroMemory(pSearchCardResult, sizeof(tagTHSearchCardResult));
...
BYTE CThirteenLogic::SearchLineCardType(const BYTE cbHandCardData[], BYTE cbHandCardCount, BYTE cbLineCount, tagTHSearchCardResult *pSearchCardResult)
{
	//设置结果
	if( pSearchCardResult )
		ZeroMemory(pSearchCardResult, sizeof(tagTHSearchCardResult));
	...
	//分析扑克
	tagTHDistributing Distributing;
	ZeroMemory(&Distributing, sizeof(Distributing));
...
//搜索同花顺
BYTE CThirteenLogic::SearchSameColorLineType(const BYTE cbHandCardData[], BYTE cbHandCardCount, BYTE cbLineCount, tagTHSearchCardResult *pSearchCardResult)
{
	//设置结果
	if (pSearchCardResult)
		ZeroMemory(pSearchCardResult, sizeof(tagTHSearchCardResult));
	...
	//变量定义
	tagTHSearchCardResult tagTempResult, tagTempResult2;
...
//构造扑克
BYTE CThirteenLogic::MakeCardData(BYTE cbValueIndex, BYTE cbColorIndex)
{
...
****************************************************/
int CThirteenLogic::CardTypeToWeights(BYTE bType, BYTE cbIndex)
{
	switch(bType)
	{
	case TH_INVALID:													//错误类型
		{
			return -1;
		}
	case TH_SINGLE:														//单牌类型
	...
	case TH_FIVE_SINGLE_LINE:										//普通顺子
		{
			return 5;
		}
	case TH_FIVE_FLUSH:													//同花
		{
			return 8;
		}
	case TH_FIVE_THREE_DEOUBLE:											//三条一对
...
****************************************************/
void CThirteenLogic::OptimizationCombo(BYTE bInFirstList[], BYTE bInNextList[], BYTE bFirstCount, BYTE bNextCount)
{
