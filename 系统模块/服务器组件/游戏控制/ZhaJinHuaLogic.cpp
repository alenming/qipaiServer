...
//扑克数据
BYTE CZhaJinHuaLogic::m_cbCardListData[52] =
{
...
//构造函数
CZhaJinHuaLogic::CZhaJinHuaLogic()
{
}
...
//获取类型
BYTE CZhaJinHuaLogic::GetCardType(BYTE cbCardData[], BYTE cbCardCount)
{
	...
	if (cbCardCount==MAX_COUNT)
	{
		...
		BYTE cbFirstColor = GetCardColor(cbTempCardData[0]);
		BYTE cbFirstValue = GetCardLogicValue(cbTempCardData[0]);
		...
		{
			if (GetCardValue(cbTempCardData[i]) == 2)	bTwo = true;
			else if (GetCardValue(cbTempCardData[i]) == 3)bThree = true;
			else if (GetCardValue(cbTempCardData[i]) == 5)bFive = true;
		}	
...
//排列扑克
void CZhaJinHuaLogic::SortCardList(BYTE cbCardData[], BYTE cbCardCount)
{
	//转换数值
...
//混乱扑克
void CZhaJinHuaLogic::RandCardList(BYTE cbCardBuffer[], BYTE cbBufferCount)
{
...
//逻辑数值
BYTE CZhaJinHuaLogic::GetCardLogicValue(BYTE cbCardData)
{
	//扑克属性
...
//对比扑克
BYTE CZhaJinHuaLogic::CompareCard(BYTE cbFirstData[], BYTE cbNextData[], BYTE cbCardCount)
{
	//设置变量
