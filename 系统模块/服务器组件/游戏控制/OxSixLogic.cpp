...
//扑克数据
BYTE COxSixLogic::m_cbCardListDataNoKing[52] =
{
...
//扑克数据
BYTE COxSixLogic::m_cbCardListDataHaveKing[54] =
{
	0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D,		//方块 A - K
	0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D,		//梅花 A - K
	0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2A, 0x2B, 0x2C, 0x2D,		//红桃 A - K
...
//获取类型
BYTE COxSixLogic::GetCardType(BYTE cbCardData[], BYTE cbCardCount)
{
	//数据效验
	ASSERT(cbCardCount == MAX_CARDCOUNT);

	//分析扑克
	tagOxSixAnalyseResult AnalyseResult;
...
//获取倍数
BYTE COxSixLogic::GetTimes(BYTE cbCardData[], BYTE cbCardCount)
{
...
void COxSixLogic::SortCardList(BYTE cbCardData[], BYTE cbCardCount, bool bDescend)
{
	//转换数值
...
//混乱扑克
void COxSixLogic::RandCardList(BYTE cbCardBuffer[], BYTE cbBufferCount)
{
	//混乱准备
...
//删除扑克
bool COxSixLogic::RemoveCard(const BYTE cbRemoveCard[], BYTE cbRemoveCount, BYTE cbCardData[], BYTE cbCardCount)
{
	//检验数据
...
//逻辑数值(新加牌型使用)
BYTE COxSixLogic::GetCardLogicValue(BYTE cbCardData)
{
	//扑克属性
...
//对比扑克
bool COxSixLogic::CompareCard(BYTE cbFirstData[], BYTE cbNextData[], BYTE cbCardCount)
{
	...
	//分析扑克
	tagOxSixAnalyseResult FirstAnalyseResult;
	tagOxSixAnalyseResult NextAnalyseResult;
...
//分析扑克
bool COxSixLogic::AnalysebCardData(const BYTE cbCardData[], BYTE cbCardCount, tagOxSixAnalyseResult & AnalyseResult)
{
	//设置结果
	ZeroMemory(&AnalyseResult, sizeof(AnalyseResult));
...
int COxSixLogic::RetType(int itype)
{
	itype = itype % 10;
	switch (itype)
	{
