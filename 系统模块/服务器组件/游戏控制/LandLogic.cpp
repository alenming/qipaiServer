...
//扑克数据
const BYTE	CLandLogic::m_cbCardData[FULL_COUNT] =
{
...
//扑克数据
const BYTE	CLandLogic::m_cbGoodCardData[34] =
{
	0x01, 0x02, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D,	//方块 A - K
	0x11, 0x12, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D,	//梅花 A - K
	0x21, 0x22, 0x28, 0x29, 0x2A, 0x2B, 0x2C, 0x2D,	//红桃 A - K
	0x31, 0x32, 0x38, 0x39, 0x3A, 0x3B, 0x3C, 0x3D,	//黑桃 A - K
	0x4E, 0x4F,
};
...
VOID CLandLogic::RandCardList(BYTE cbCardBuffer[], BYTE cbBufferCount)
{
	...
	do
	{
		cbPosition = random() % (cbBufferCount - cbRandCount);
...
//混乱好牌
VOID CLandLogic::RandGoodCardList(BYTE cbCardBuffer[], BYTE cbBufferCount)
{
	//混乱准备
	BYTE cbCardData[CountArray(m_cbGoodCardData)];

	BYTE cbAllCount = CountArray(m_cbGoodCardData);


	for (int i = 0; i < 100; i++)
	{
		BYTE cbTempCardData[LAND_NORMAL_COUNT] = {0};
		...
		do
		{
			cbPosition = random() % (cbAllCount - cbRandCount);
			cbTempCardData[cbRandCount++] = cbCardData[cbPosition];
			cbCardData[cbPosition] = cbCardData[cbAllCount - cbRandCount];
		} while (cbRandCount < cbBufferCount);

		tagAnalyseResult AnalyseResult = {};
		AnalysebCardData(cbTempCardData, LAND_NORMAL_COUNT, AnalyseResult);

		CopyMemory(cbCardBuffer, cbTempCardData, sizeof(BYTE) * LAND_NORMAL_COUNT);
...
//删除扑克
bool CLandLogic::RemoveCardList(const BYTE cbRemoveCard[], BYTE cbRemoveCount, BYTE cbCardData[], BYTE cbCardCount)
{
	//检验数据
...
//删除扑克
bool CLandLogic::RemoveCard(const BYTE cbRemoveCard[], BYTE cbRemoveCount, BYTE cbCardData[], BYTE cbCardCount)
{
	//检验数据
	ASSERT(cbRemoveCount<=cbCardCount);
...
//排列扑克
VOID CLandLogic::SortOutCardList(BYTE cbCardData[], BYTE cbCardCount)
{
	//获取牌型
...
//对比扑克
bool CLandLogic::CompareCard(const BYTE cbFirstCard[], const BYTE cbNextCard[], BYTE cbFirstCount, BYTE cbNextCount)
{
	//获取类型
...
//构造扑克
BYTE CLandLogic::MakeCardData(BYTE cbValueIndex, BYTE cbColorIndex)
{
...
//出牌搜索
BYTE CLandLogic::SearchOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, const BYTE cbTurnCardData[], BYTE cbTurnCardCount,
...
//同牌搜索
BYTE CLandLogic::SearchSameCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, BYTE cbReferCard, BYTE cbSameCardCount,
								tagSearchCardResult *pSearchCardResult )
{
	//设置结果
...
//带牌类型搜索(三带一，四带一等)
BYTE CLandLogic::SearchTakeCardType(const BYTE cbHandCardData[], BYTE cbHandCardCount, BYTE cbReferCard, BYTE cbSameCount, BYTE cbTakeCardCount,
									tagSearchCardResult *pSearchCardResult )
{
	//设置结果
	if( pSearchCardResult )
...
//连牌搜索
BYTE CLandLogic::SearchLineCardType(const BYTE cbHandCardData[], BYTE cbHandCardCount, BYTE cbReferCard, BYTE cbBlockCount, BYTE cbLineCount,
									tagSearchCardResult *pSearchCardResult )
{
	//设置结果
	if( pSearchCardResult )
...
//搜索飞机
BYTE CLandLogic::SearchThreeTwoLine(const BYTE cbHandCardData[], BYTE cbHandCardCount, tagSearchCardResult *pSearchCardResult)
{
	//设置结果
	if( pSearchCardResult )
		ZeroMemory(pSearchCardResult,sizeof(tagSearchCardResult));
