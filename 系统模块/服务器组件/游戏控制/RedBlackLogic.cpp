...
//扑克数据
BYTE CRedBlackLogic::m_cbCardListData[52] =
{
...
//获取类型
BYTE CRedBlackLogic::GetCardType(BYTE cbCardData[], BYTE cbCardCount)
{
	...
	if (cbCardCount==MAX_COUNT)
	{
		...
		//顺金类型
		if ((cbSameColor) && (bLineCard)) return CT_RB_SHUN_JIN;

		//顺子类型
		if ((!cbSameColor) && (bLineCard)) return CT_RB_SHUN_ZI;

		//金花类型
		if ((cbSameColor) && (!bLineCard)) return CT_RB_JIN_HUA;
...
//排列扑克
void CRedBlackLogic::SortCardList(BYTE cbCardData[], BYTE cbCardCount)
{
	//转换数值
...
//混乱扑克
void CRedBlackLogic::RandCardList(BYTE cbCardBuffer[], BYTE cbBufferCount)
{
...
//逻辑数值
BYTE CRedBlackLogic::GetCardLogicValue(BYTE cbCardData)
{
	//扑克属性
...
//对比扑克
BYTE CRedBlackLogic::CompareCard(BYTE cbFirstData[], BYTE cbNextData[], BYTE cbCardCount)
{
	//设置变量
	...
	//特殊情况分析
	if ((cbNextType + cbFirstType) == (CT_RB_SPECIAL + CT_RB_BAO_ZI))return (BYTE)(cbFirstType>cbNextType);

	//还原单牌类型
	if (cbNextType == CT_RB_SPECIAL)cbNextType = CT_RB_SINGLE;
	if (cbFirstType == CT_RB_SPECIAL)cbFirstType = CT_RB_SINGLE;
	...
	switch(cbFirstType)
	{
	case CT_RB_BAO_ZI:			//豹子
	case CT_RB_SINGLE:			//单牌
	case CT_RB_JIN_HUA:		//金花
		{
			//对比数值
			for (BYTE i=0;i<cbCardCount;i++)
			{
	...
	case CT_RB_SHUN_ZI:		//顺子
	case CT_RB_SHUN_JIN:		//顺金 432>A32
		{		
	...
	case CT_RB_DOUBLE_SHINE:
	case CT_RB_DOUBLE:			//对子
		{
			BYTE cbNextValue=GetCardLogicValue(NextData[0]);
			BYTE cbFirstValue=GetCardLogicValue(FirstData[0]);
