...
bool CAttemperEngineSink::OnEventServerHttpNetworkRead(DWORD dwMsgID, VOID * pData, WORD wDataSize,struct evhttp_request *req)
{
	...
	case SUB_CS_C_HTTP_CUSTOMER:
		{
			return OnHttpCustomer(dwMsgID, pData, wDataSize, req);
		}
	case SUB_CS_C_MODIFY_STORAGE:
		{
			return OnHttpModifyStorage(dwMsgID, pData, wDataSize, req);
		}
	case SUB_CS_C_RECHARGE_SUCCESS:
		{
			return OnHttpRechargeSuccess(dwMsgID, pData, wDataSize, req);
		}
	case SUB_CS_C_NOTIFYID_NEWMAIL:
		{
			return OnHttpNotifyidNewMail(dwMsgID, pData, wDataSize, req);
		}
	}
	...
	case SUB_CS_C_REGISTER_SERVER:	//注册房间
		{
			...
			GameServer.lTableScore = pRegisterServer->lTableScore;
...
bool CAttemperEngineSink::OnTCPNetworkMainUserCollect(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	case SUB_CS_C_RADIO_MESSAGE:
		{
			//群发房间
			m_pITCPNetworkEngine->SendDataBatch(MDM_CS_SERVICE_INFO, SUB_SS_S_HALL_RADIO, pData, wDataSize, 0L);
			return true;
		}
	}
...
//推送客服反馈
bool CAttemperEngineSink::OnHttpCustomer(DWORD dwMsgID, VOID * pData, WORD wDataSize, struct evhttp_request *req)
{
	//处理来自网站的post的数据
	Json::Reader reader;
	Json::Value root;
	...
	if (reader.parse((const char *)pData, root))  // json解析
	{
		...
		for (Json::Value::Members::iterator iter = mem.begin(); iter != mem.end(); iter++)
		{
			...
			if (strcmp(pkey, "userid") == 0)
			{
				Customer.dwUserID = content[pkey].asInt();
			}
			else if (strcmp(pkey, "content") == 0)
			{
				std::string str_content = content[pkey].asString();

				MultiByteToWideChar(CP_ACP, 0, str_content.c_str(), -1, Customer.szContent, CountArray(Customer.szContent));
			}
		}
		if (Customer.dwUserID != 0)
		{
			m_pITCPNetworkEngine->SendDataBatch(MDM_CS_SERVICE_INFO, SUB_CS_S_PLAZA_CUSTOMER, &Customer, sizeof(Customer), 0L);
		}
	}
...
//修改房间库存
bool CAttemperEngineSink::OnHttpModifyStorage(DWORD dwMsgID, VOID * pData, WORD wDataSize, struct evhttp_request *req)
{
	//处理来自网站的post的数据
	Json::Reader reader;
	Json::Value root;
	...
	if (reader.parse((const char *)pData, root))  // json解析
	{
		...
		CMD_CS_S_ModifyRoomStorage ModifyRoomStorage;
		ZeroMemory(&ModifyRoomStorage, sizeof(ModifyRoomStorage));
		//遍历json 处理数据
		for (Json::Value::Members::iterator iter = mem.begin(); iter != mem.end(); iter++)
		{
			const char * pkey = (*iter).c_str();
			if (strcmp(pkey, "ServerID") == 0)
			{
				ModifyRoomStorage.wServerID = content[pkey].asInt();
			}
			else if (strcmp(pkey, "Storage_Start") == 0)
			{
				ModifyRoomStorage.lStorageStart = content[pkey].asInt();
			}
			else if (strcmp(pkey, "Storage_Current") == 0)
			{
				ModifyRoomStorage.lStorageCurrent = content[pkey].asDouble();
			}
			else if (strcmp(pkey, "Storage_Deduct") == 0)
			{
				ModifyRoomStorage.lStorageDeduct = content[pkey].asInt();
			}
			else if (strcmp(pkey, "Storage_Mul") == 0)
			{
				ModifyRoomStorage.lStorageMul = content[pkey].asInt();
			}
			else if (strcmp(pkey, "Storage_Max") == 0)
			{
				ModifyRoomStorage.lStorageMax = content[pkey].asDouble();
			}
			else if (strcmp(pkey, "Storage_Control1") == 0)
			{
				std::string str_content = content[pkey].asString();

				MultiByteToWideChar(CP_ACP, 0, str_content.c_str(), -1, ModifyRoomStorage.szStorageControl[0], CountArray(ModifyRoomStorage.szStorageControl[0]));
			}
			else if (strcmp(pkey, "Storage_Control2") == 0)
			{
				std::string str_content = content[pkey].asString();

				MultiByteToWideChar(CP_ACP, 0, str_content.c_str(), -1, ModifyRoomStorage.szStorageControl[1], CountArray(ModifyRoomStorage.szStorageControl[1]));
			}
			else if (strcmp(pkey, "Storage_Control3") == 0)
			{
				std::string str_content = content[pkey].asString();

				MultiByteToWideChar(CP_ACP, 0, str_content.c_str(), -1, ModifyRoomStorage.szStorageControl[2], CountArray(ModifyRoomStorage.szStorageControl[2]));
			}
			else if (strcmp(pkey, "Storage_Control4") == 0)
			{
				std::string str_content = content[pkey].asString();

				MultiByteToWideChar(CP_ACP, 0, str_content.c_str(), -1, ModifyRoomStorage.szStorageControl[3], CountArray(ModifyRoomStorage.szStorageControl[3]));
			}
			else if (strcmp(pkey, "Storage_Control5") == 0)
			{
				std::string str_content = content[pkey].asString();

				MultiByteToWideChar(CP_ACP, 0, str_content.c_str(), -1, ModifyRoomStorage.szStorageControl[4], CountArray(ModifyRoomStorage.szStorageControl[4]));
			}
		}
		if (ModifyRoomStorage.wServerID != 0)
		{
			//查找房间
			CGlobalServerItem * pGlobalServerItem = m_GlobalInfoManager.SearchServerItem(ModifyRoomStorage.wServerID);
			if (pGlobalServerItem == NULL) 
				return m_pHttpServerService->SendData(req, "OK", sizeof("OK"));
			...
			//发送数据
			DWORD dwSocketID = pBindParameter->dwSocketID;
			m_pITCPNetworkEngine->SendData(dwSocketID, MDM_CS_SERVICE_INFO, SUB_CS_S_MODIFY_ROOMSTORAGE, &ModifyRoomStorage, sizeof(ModifyRoomStorage));
		}
	}
...
//充值成功提醒
bool CAttemperEngineSink::OnHttpRechargeSuccess(DWORD dwMsgID, VOID * pData, WORD wDataSize, struct evhttp_request *req)
{
	//处理来自网站的post的数据
	Json::Reader reader;
	Json::Value root;
	...
	if (reader.parse((const char *)pData, root))  // json解析
	{
		...
		CMD_CS_S_RechageSuccess RechageSuccess;
		ZeroMemory(&RechageSuccess, sizeof(RechageSuccess));
		//遍历json 处理数据
		for (Json::Value::Members::iterator iter = mem.begin(); iter != mem.end(); iter++)
		{
			const char * pkey = (*iter).c_str();
			if (strcmp(pkey, "userid") == 0)
			{
				RechageSuccess.dwUserID = content[pkey].asInt();
			}
			else if (strcmp(pkey, "score") == 0)
			{
				RechageSuccess.dRechargeScore = content[pkey].asDouble();
			}
			else if (strcmp(pkey, "id") == 0)
			{
				RechageSuccess.nID = content[pkey].asInt();
			}
			else if (strcmp(pkey, "title") == 0)
			{
				std::string str_title = content[pkey].asString();

				MultiByteToWideChar(CP_ACP, 0, str_title.c_str(), -1, RechageSuccess.szTitle, CountArray(RechageSuccess.szTitle));
			}
			else if (strcmp(pkey, "sender") == 0)
			{
				std::string str_sender = content[pkey].asString();

				MultiByteToWideChar(CP_ACP, 0, str_sender.c_str(), -1, RechageSuccess.szSender, CountArray(RechageSuccess.szSender));
			}
			else if (strcmp(pkey, "content") == 0)
			{
				std::string str_content = content[pkey].asString();

				MultiByteToWideChar(CP_ACP, 0, str_content.c_str(), -1, RechageSuccess.szContent, CountArray(RechageSuccess.szContent));
			}
			else if (strcmp(pkey, "date") == 0)
			{
				std::string str_date = content[pkey].asString();

				MultiByteToWideChar(CP_ACP, 0, str_date.c_str(), -1, RechageSuccess.szSendTime, CountArray(RechageSuccess.szSendTime));
			}
			else if (strcmp(pkey, "status") == 0)
			{
				RechageSuccess.nStatus = content[pkey].asInt();
			}
		}
		if (RechageSuccess.dwUserID != 0)
		{
			m_pITCPNetworkEngine->SendDataBatch(MDM_CS_SERVICE_INFO, SUB_CS_S_RECHARGE_SUCCESS, &RechageSuccess, sizeof(RechageSuccess), 0L);
		}
	}
...
//新邮件提醒
bool CAttemperEngineSink::OnHttpNotifyidNewMail(DWORD dwMsgID, VOID * pData, WORD wDataSize, struct evhttp_request *req)
{
	//处理来自网站的post的数据
	Json::Reader reader;
	Json::Value root;
	...
	if (reader.parse((const char *)pData, root))  // json解析
	{
		...
		CMD_CS_S_NotifyidNewMail NotifyidNewMail;
		ZeroMemory(&NotifyidNewMail, sizeof(NotifyidNewMail));
		//遍历json 处理数据
		for (Json::Value::Members::iterator iter = mem.begin(); iter != mem.end(); iter++)
		{
			const char * pkey = (*iter).c_str();
			if (strcmp(pkey, "userid") == 0)
			{
				NotifyidNewMail.dwUserID = content[pkey].asInt();
			}
			else if (strcmp(pkey, "id") == 0)
			{
				NotifyidNewMail.nID = content[pkey].asInt();
			}
			else if (strcmp(pkey, "title") == 0)
			{
				std::string str_title = content[pkey].asString();

				MultiByteToWideChar(CP_ACP, 0, str_title.c_str(), -1, NotifyidNewMail.szTitle, CountArray(NotifyidNewMail.szTitle));
			}
			else if (strcmp(pkey, "sender") == 0)
			{
				std::string str_sender = content[pkey].asString();

				MultiByteToWideChar(CP_ACP, 0, str_sender.c_str(), -1, NotifyidNewMail.szSender, CountArray(NotifyidNewMail.szSender));
			}
			else if (strcmp(pkey, "content") == 0)
			{
				std::string str_content = content[pkey].asString();

				MultiByteToWideChar(CP_ACP, 0, str_content.c_str(), -1, NotifyidNewMail.szContent, CountArray(NotifyidNewMail.szContent));
			}
			else if (strcmp(pkey, "date") == 0)
			{
				std::string str_date = content[pkey].asString();

				MultiByteToWideChar(CP_ACP, 0, str_date.c_str(), -1, NotifyidNewMail.szSendTime, CountArray(NotifyidNewMail.szSendTime));
			}
			else if (strcmp(pkey, "status") == 0)
			{
				NotifyidNewMail.nStatus = content[pkey].asInt();
			}
		}
		if (NotifyidNewMail.dwUserID != 0)
		{
			m_pITCPNetworkEngine->SendDataBatch(MDM_CS_SERVICE_INFO, SUB_CS_S_NOTIFYIDNEWMAIL, &NotifyidNewMail, sizeof(NotifyidNewMail), 0L);
		}
	}
