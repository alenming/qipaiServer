...
bool CAndroidUserItem::SendSocketData(WORD wSubCmdID)
{
	...
	//模式判断
	if ((m_AndroidItemConfig.pAndroidParameter->dwServiceMode&ANDROID_THEJOINT)!=0L) return true;;
	//发送数据
...
bool CAndroidUserItem::SendSocketData(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
	...
	ASSERT(m_pIServerUserItem!=NULL);
	if (m_pIServerUserItem==NULL) return false;
	DWORD dwTheJoint =m_AndroidItemConfig.pAndroidParameter->dwServiceMode&ANDROID_THEJOINT;
	//模式判断
...
//发送准备
bool CAndroidUserItem::SendAndroidQuit(VOID * pData, WORD wDataSize)
{
	//状态效验
	//	ASSERT((m_pIServerUserItem!=NULL)&&(m_pIServerUserItem->GetUserStatus()==US_SIT));
	if ((m_pIServerUserItem==NULL)) return false;
	...
	//发送准备
	DWORD dwAndroidID=MAKELONG(m_wAndroidIndex,m_wRoundID);
	m_pIAndroidUserManager->SendDataToServer(dwAndroidID,MDM_GF_FRAME,SUB_GF_ANDEOID_QUIT,pData,wDataSize);
...
bool CAndroidUserItem::SendChatMessage(DWORD dwTargetUserID, LPCTSTR pszChatString, COLORREF crColor)
{
	...
	UserChat.wItemIndex = 0;

	//发送命令
	DWORD dwAndroidID = MAKELONG(m_wAndroidIndex, m_wRoundID);
	m_pIAndroidUserManager->SendDataToServer(dwAndroidID, MDM_GF_FRAME, SUB_GF_USER_CHAT, &UserChat, sizeof(UserChat));
