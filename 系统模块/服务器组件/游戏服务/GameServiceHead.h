...
//常量定义
#define RESERVE_USER_COUNT			10L									//保留数目
...
struct tagGameServiceOption
{
	...
	tagRoomStorageOption 			pRoomStorageOption;					//服务配置
};
...
public:
	...
	//设置控制
	virtual VOID SetRoomGameRate(int bGameRate) = NULL;
	//设置控制
	virtual int GetRoomGameRate()=NULL;
	//设置控制
...
public:
	...
	//广播大厅消息
	virtual bool SendHallRadioMessage(VOID * pData, WORD wDataSize) = NULL;
	//群发数据给游戏房间内每一个旁观玩家(没坐在桌子上的)
	virtual bool SendTableDataToLookOnUser(WORD wSubCmdID, VOID * pData, WORD wDataSize) = NULL;

public:
	//获取控制结果
	virtual VOID * GetControlResult(VOID * pData, WORD wDataSize, VOID * pResult) = NULL;
	//计算库存衰减
	virtual VOID CalculateStorageDeduct() = NULL;
...
public:
	...
	//百人游戏获取游戏记录
	virtual void OnGetGameRecord(VOID *GameRecord) = NULL;

	//获取百人游戏是否下注状态而且玩家下注了
	virtual bool OnGetUserBetInfo(WORD wChairID, IServerUserItem * pIServerUserItem) = NULL;
...
public:
	...
	//群发数据给游戏房间内每一个旁观玩家(没坐在桌子上的)
	virtual bool SendDataToLookOnUser(WORD wSubCmdID, VOID * pData, WORD wDataSize) = NULL;
...
public:
	...
	//解锁数据库积分锁定
	virtual VOID UnLockScoreLockUser(DWORD dwUserID, DWORD dwInoutIndex, DWORD dwLeaveReason) = NULL;
};
