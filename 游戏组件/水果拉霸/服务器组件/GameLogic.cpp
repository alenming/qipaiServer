...
//混乱扑克
VOID CGameLogic::RandCardList(BYTE cbCardBuffer[][ITEM_X_COUNT])
{
...
//取得中奖
int CGameLogic::GetZhongJiangTime(BYTE cbItemInfo[][ITEM_X_COUNT])
{
	DOUBLE nTime = 0;
	...
	for (int i = 0; i < LINE_COUNT; i++)
	{
		...
		if (cbWildLinkCount == ITEM_X_COUNT)
			cbLeftLinkCount == ITEM_X_COUNT;

		if ((cbWildLinkCount + 1) < ITEM_X_COUNT)
		{
			for (int j = cbWildLinkCount + 1; j < ITEM_X_COUNT; j++)
			{
				BYTE cbIndex = m_cbLineInfo[i][j];
		...
		if (cbWildLinkCount > 0 && cbWildLinkCount < ITEM_X_COUNT)
		{
...
//获取免费次数
BYTE CGameLogic::GetEnterFree(BYTE cbItemInfo[][ITEM_X_COUNT])
{
	BYTE nTime = 0;
...
//获取JackPot分数
SCORE CGameLogic::GetJackPotScore(BYTE cbItemInfo[][ITEM_X_COUNT], SCORE lJackPotScore)
{
	SCORE nTime = 0;
