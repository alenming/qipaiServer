	...
	case IDI_CHECK_BANKER:		//检查上庄
		{
				...
				&& m_stnApplyCount < 2 )
			{
				...
				if (pIUserItemBanker->GetUserScore() > m_lBankerCondition ) 
				{
					...
					m_stlApplyBanker++;
					m_stnApplyCount++;
					m_pIAndroidUserItem->SetGameTimer(IDI_REQUEST_BANKER, (rand() % (m_cbTimeLeave+1)) + 1);
				}
				else
				{
					//执行取款
	...
	default:
		{
			if (nTimerID >= IDI_PLACE_JETTON && nTimerID <= IDI_PLACE_JETTON+MAX_CHIP_TIME)
			{
				...
				{
					if (m_RobotInfo.nChip[i] == m_lRobotJettonLimit[1])
						nMaxIndex = i;
					if (m_RobotInfo.nChip[i] == m_lRobotJettonLimit[0])
						nMinIndex = i;
				}
				...
				//只有5个下注区域，防止错误
				int nCurChip = rand() % (nMaxIndex - nMinIndex+1) + nMinIndex;

				//变量定义
				CMD_C_PlaceJetton PlaceJetton = {};

				//构造变量
				PlaceJetton.cbJettonArea = rand() % 4;		//区域宏从1开始
...
{	
	...
	for (int i = 0; i < m_nChipTime; i++)
	{
		...
		if (nElapse < 5 || nElapse > pGameStart->cbTimeLeave)	continue;
