...
*/
//Multiple 比较出来的倍数
int CGameLogic::CompareCard(const BYTE cbFirstCardData[], BYTE cbFirstCardCount, const BYTE cbNextCardData[], BYTE cbNextCardCount, int nMaxTimes, BYTE &Multiple)
{
	//合法判断
	...
	if (cbFirstCardType != cbNextCardType) 
	{
		if (cbNextCardType > cbFirstCardType)
		{
			if (nMaxTimes == 10)
			{
				if (cbNextCardType >= CT_SPECIAL_NIUNIU)
					Multiple = 10;
				else if (cbNextCardType >= CT_SPECIAL_NIU2)
		...
		else
		{
			if (nMaxTimes == 10)
			{
				if (cbFirstCardType >= CT_SPECIAL_NIUNIU)
					Multiple = 10;
				else if (cbFirstCardType >= CT_SPECIAL_NIU2)
