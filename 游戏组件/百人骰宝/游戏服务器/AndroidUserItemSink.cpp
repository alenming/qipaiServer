	...
	default:
		{
			if (nTimerID >= IDI_PLACE_JETTON && nTimerID <= IDI_PLACE_JETTON+MAX_CHIP_TIME)
			{
				...
				//变量定义
				int nRandNum = 0, nChipArea = 0, nCurChip = 0, nCurJetLmt[2] = {};
				SCORE lMaxChipLmt = m_pIAndroidUserItem->GetMeUserItem()->GetUserScore();     //最大可下注值
		        ...
		        //构造变量
		        PlaceJetton.cbJettonType = nChipArea;   //区域宏从1开始
	...
	case GAME_STATUS_FREE:			//空闲状态
		{
			...
			for (int i = 0; i < 5; i++)
				m_RobotInfo.nChip[i] = pStatusFree->lChips[i];
